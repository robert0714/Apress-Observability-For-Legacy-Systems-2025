/* 
 * Copyright (c) 2002-2023 Cloud Software Group, Inc. 
 * All Rights Reserved. 
 * 
 */

/*
 * This is a simple sample of TLS connectivity for
 * EMS clients. It demonstrates both TLS-based JNDI
 * lookups as well as TLS-based client/server communications.
 *
 * This sample establishes a TLS connection, sends and
 * receives back a single message.
 *
 * If not specified this sample assumes a provider URL of
 *                  "tibjmsnaming://localhost:7243".
 * Note that your TIBCO EMS server must have been configured with a TLS port
 * of 7243.
 *
 ...
 *                        create TLS-based connections. It must have previously
...
public class tibjmsSSLJNDI
{
     ...
     * the current time.  It allows the samples that use TLS to initialize
     * the TLS environment much faster than if they had to generate a truly
     * random seed.
     *
    ...
    public tibjmsSSLJNDI(String[] args)
    {
        ...
        System.err.println("TLS with JNDI sample.");

        // initialize TLS environment
        ...
        try
        {
            ...
            if (ssl_identity != null)
            {
                ...
                if (ssl_private_key != null)
                   environment.put(TibjmsContext.SSL_PRIVATE_KEY, ssl_private_key);
            }

            if (disable_verify_hostname)
              environment.put(TibjmsContext.SSL_ENABLE_VERIFY_HOST_NAME, new Boolean(false));

            if (disable_verify_host)
        ...
        try
        {
            ...
            // use TLS to lookup the ConnectionFactory
            ...
            message.setStringProperty("field","TLS message");
    ...
    void usage()
    {
        ...
        System.err.println("");
        System.err.println(" -ssl_vendor       <name>      - TLS vendor: 'j2se' (the default)");
        System.err.println(" -ssl_trace                    - trace TLS initialization");
        System.err.println(" -ssl_debug_trace              - trace TLS handshake and related");
        ...
        System.err.println(" -ssl_no_verify_hostname       - disable host name verification");
        System.err.println(" -ssl_no_verify_host           - disable host verification");
        System.exit(0);
    }
    ...
    void parseArgs(String[] args)
    {
        ...
        while (i < args.length)
        {
            ...
            if (args[i].compareTo("-ssl_no_verify_hostname")==0)
            {
                disable_verify_hostname = true;
                i += 1;
            }
            else
            if (args[i].compareTo("-ssl_no_verify_host")==0)
            {
